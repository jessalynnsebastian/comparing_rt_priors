...
//log-scale integrated brownian motion prior on Rt
//lognormal prior on sigma
functions{
...
transformed parameters{
  ...
  for (r in 2:n){
    ...
    // do Y = Sigma^1/2 * Z + mu
    log_rt[r] = ([[sqrt(sigma), sigma^(3/2)/2], [sigma^(3/2)/2, sigma^(3/2)/(sqrt(12))]]*(Z[r-1])')' + [ log_rt[r-1,1], log_rt[r-1,2] + sigma*log_rt[r-1,1]];
  }
